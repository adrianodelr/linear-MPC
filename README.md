...
```
<!-- $\mathbf{e}_{k}=\mathbf{r}_{k}-\mathbf{x}_{k}$ -->
Here $ = \mathbf{r}_{k} - \mathbf{x}_{k}$ is the difference between the desired reference and the predicted state at time step $k$; and $\mathbf{u}_{k}$ the control vector applied to the system. The diagonal matrices $\mathbf{Q}$ and $\mathbf{R}$ are used to determine 
